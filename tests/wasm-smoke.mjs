...
function exerciseEnigmaXmlInput(bytes, name, label) {
  withAllocatedInput(bytes, name, (dataPointer, namePointer) => {
    ...
    try {
      assertResult(
        Module._denigma_convert(dataPointer, bytes.byteLength, namePointer, 0, 0, 0, 0, 0, 2, 0, selectionPointer, 1),
        `${label} to MusicXML`, '<score-partwise', 1, false, [0]);
    } finally {
      Module._denigma_free(selectionPointer);
    }
    assertResult(
      Module._denigma_convert(dataPointer, bytes.byteLength, namePointer, 1, 0, 0, 0, 0, 2, 0, 0, 0),
      `${label} to MNX`, '"mnx"');
    assertResult(
      Module._denigma_convert(dataPointer, bytes.byteLength, namePointer, 2, 0, 0, 0, 0, 2, 0, 0, 0),
...
try {
  ...
  try {
    assertResult(
      Module._denigma_convert(inputPointer, input.byteLength, sourcePointer, 0, 0, 1, 0, 0, 2, 0, selectionPointer, 1),
    ...
    if (firstPartOutputIndex !== undefined) {
      ...
      assertResult(
        Module._denigma_convert(inputPointer, input.byteLength, sourcePointer, 0, 1, 0, 0, 0, 2, 0, selectionPointer, 1),
      ...
      assertResult(
        Module._denigma_convert(inputPointer, input.byteLength, sourcePointer, 0, 0, 0, 0, 0, 2, 0, selectionPointer, 2),
  ...
  assertResult(
    Module._denigma_convert(inputPointer, input.byteLength, sourcePointer, 1, 0, 0, 0, 0, 2, 0, 0, 0),
    'MNX with verbose logging', '"mnx"', 1, true);
  const enigmaXml = assertResult(
    Module._denigma_convert(inputPointer, input.byteLength, sourcePointer, 2, 0, 0, 0, 0, 2, 0, 0, 0),
    'EnigmaXML', '<finale');
